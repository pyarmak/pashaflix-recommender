...
export const getSeasonsApi = (id: number) => {
  return axios.get<Season[]>(
    `${BASE_URL}/shows/${id}/seasons?translations=en`,
    {
      headers: base_headers,
    },
  );
...
export const getSeasonEpisodesApi = (id: number, season: number) => {
  return axios.get<Episode[]>(
    `${BASE_URL}/shows/${id}/seasons/${season}?extended=full&translations=en`,
    {
      headers: base_headers,
    },
  );
...
export const getRequests = <T extends RequestedMovie | RequestedShow>(
  type: ItemType,
) => {
...
export const requestMovie = (id: number) => {
  return axios.post(
    `https://recommend.yarmak.me/api/v1/Request/movie`,
    {
      theMovieDbId: id,
      languageCode: 'en',
    },
...
export const getTranslationsApi = (id: number, type: ItemType) => {
  return axios.get<Translation[]>(
    `${BASE_URL}/${type}s/${id}/translations/en`,
    {
      headers: base_headers,
    },
  );
...
export const getRecommendedApi = <T extends RecommendedMovie | ShowWatched>(
  session: Session,
  type: ItemType,
) => {
  const url =
    type === 'movie'
      ? `${BASE_URL}/recommendations/movies?ignore_collected=false`
      : `${BASE_URL}/recommendations/shows?ignore_collected=false`;
